...
_INSTRUCTIONS = (
    ...
    "orientation the user gave. Lay rooms out so they actually form one "
    "cohesive building: start the first room at the plot boundary's corner "
    "(plus a small setback margin, e.g. 5 ft, if you don't have a specific "
    "setback requirement yet), then place each next room by ADDING the "
    "previous room's width or depth to get its x or y -- e.g. room 2's x = "
    "room 1's x + room 1's width_ft, to sit flush against room 1 with zero "
    "gap -- rather than inventing separate coordinates for each room. Never "
    "leave a gap between rooms that should be adjacent, and never let a "
    "room's extent (x + width_ft, y + depth_ft) exceed the plot boundary. "
    "If you don't have an explicit adjacency layout in mind, default to a "
    "simple single row (each room to the right of the last) or a compact "
    "grid within the plot -- anything cohesive beats scattered rooms with "
    "gaps. Then call create_floor_plan ONCE with the full room list, "
    "instead of calling create_wall/create_room repeatedly per room. It "
    "builds walls + rooms only (no doors/windows) and returns "
    "each room's wall_ids -- add doors/windows afterward with create_door/"
    "create_window using those ids. Adjacent rooms sharing an edge get "
    "exactly ONE wall there (not one per room) -- that wall's id appears in "
    "both rooms' wall_ids lists, so a room with neighbors on every side can "
    "have fewer than 4 wall_ids of its own. "
    "After create_floor_plan (or any batch of wall/room creation), ALWAYS "
    "call check_room_overlaps and check_wall_connections before telling the "
    "user it's done. If check_room_overlaps finds an overlap, move or resize "
    "the offending room's walls with move_element/modify_wall to fix it. "
    "Any duplicate_wall_pairs or dangling_endpoints from check_wall_connections "
    "is a real problem now (walls should never overlap since create_floor_plan "
    "shares them) -- fix it with move_element/modify_wall/delete_element, "
    "then re-run the same check once to confirm before reporting success. "
    "check_building_coverage works "
    "like check_setbacks -- facts only (plot area, building footprint, "
    "coverage ratio), no verdict -- pair it with ask_building_code the same "
    "way for any FSI/coverage compliance question, and always state its "
    "actual numbers in your reply even if ask_building_code found nothing "
